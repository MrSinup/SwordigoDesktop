...
// link time. Searches both libswordigo.so (g_main_mod_64) and the loaded
// SRE version module (g_sre_mod = libsre12.so / libsre13.so) dynsym tables.
//
...
extern so_module_arm64 g_sre_mod;      // libsre12/libsre13 (version SRE module)
...
extern "C" uint64_t elf_lookup_symbol(const char* name) {
    ...
    // Fallback: search the SRE version module (e.g. for sre-internal hooks)
