...
GameSceneController *gsc_get(void) {
	return g_gsc;
}
...
HOOK_SYMBOL(
	...
	g_gsc = gsc;
	if (gsc && gsc->CharacterState) {
		g_characterState = (CharacterState *)gsc->CharacterState;
	}
	if (orig_GSC_Update_Hook) {
		orig_GSC_Update_Hook(gsc, dt);
	}
}
