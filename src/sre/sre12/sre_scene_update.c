 ...
 * symbols within libsre12.so rather than falling back to the bridge PLT stub. */
#include "sre_lua.h"
...
static int sp_vtable_is_valid(uint64_t vtable) {
    ...
    if (vtable >= 0x2000000ULL && vtable < 0x2600000ULL) return 1; /* libsre12.so + libsre-extras.so guest vtables */
