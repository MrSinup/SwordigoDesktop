/*
 * sre_init.c — libsre12.so initialization and hook table
 *
 * This file is compiled as part of libsre12.so (ARM64).
 * It provides:
 ...
 * Hook Table
 * =========================================================================
 * The host reads this table after loading libsre12.so.
 * For each entry, it writes a trampoline at swordigo_base + target_offset
 * that redirects to the symbol in libsre12.so.
 *
...
void sre_install_exception_safeguard(void) {
     ...
     *   libsre12.so), so we need its guest virtual address. */
 ...
 * Also accept SRE's vtables (libsre12.so loaded at 0x2000000):
...
int sre_is_valid_vtable_ptr(uint64_t vtable) {
    ...
    /* libsre12.so data sections (loaded at ~0x2000000) */
...
int sre_is_valid_code_ptr(uint64_t addr) {
    ...
    /* libsre12.so .text (loaded at ~0x2000000) */
