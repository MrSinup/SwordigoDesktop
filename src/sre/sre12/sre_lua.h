/* sre_lua.h — Lua API function pointer types for libsre12.so
 *
 * These are resolved by the host from libswordigo.so's symbol table
 * and passed to libsre12.so via sre_init_lua().
 *
 ...
 * (src/sre/extras/sre_ffi.c). SRE core registers only the
